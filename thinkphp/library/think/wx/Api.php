...
use think\wx\Utils\Error;
use think\wx\Utils\SHA1;
use think\wx\Utils\Xml;
use think\Log;
...
class Api 
{
    ...
    public function __construct($config)
    {
        ...
        $this->sslKeyPath               =   isset($config['sslKeyPath']) ? $config['sslKeyPath'] : false;
        $this->sslCertPath              =   isset($config['sslCertPath']) ? $config['sslCertPath'] : false;
        $this->sslrootcatPath           =   isset($config['sslrootcatPath']) ? $config['sslrootcatPath'] : false;
    }
    ...
    public function get_access_token()
    {
        ...
        $arr_token=cache('wechat_token');
        if(is_array($arr_token)==false)
        {
            $arr_token=[];
        }
        if(array_key_exists($this->appId,$arr_token)==false)
        {
            $arr_token[$this->appId]=$this->new_access_token();
            cache('wechat_token', $arr_token);
        }

        $token= $arr_token[$this->appId];
    ...
    public  function  template_send($openid,$template_id,$url='',$data)
    {
        Log::debug('发送微信消息的参数：' . print_r(func_get_args(), true));
        $xml = sprintf('{'.
            '"touser":"%s",'.
            '"template_id":"%s",'.
            '"url":"%s",'.
    ...
    public function create_qrcode1($scene_str)
    {
        $url = self::API_DOMAIN . 'cgi-bin/qrcode/create?access_token=' . $this->get_access_token();

        $xml = sprintf('{"action_name": "QR_LIMIT_STR_SCENE", "action_info": {"scene": {"scene_str": "%s"}}}',
            $scene_str);
        //echo $url."<br>";
        //print_r($xml) ;
        //echo  "<br>";
    ...
    public function get_qrcode_url($ticket='')
    {
        if($ticket=='')
        {
            $info=self::create_qrcode(999);
            if(empty($info[1])){
                $ticket = "";
            }else {
                $ticket = $info[1]->ticket;
            }
        }
     ...
     * ```
     */
    public function get_qrcode_url_by_str($scene_str='', $ticket=''){
        if($ticket=='')
        {
            $info=self::create_qrcode1($scene_str);
            if(empty($info[1])){
                $ticket = "";
            }else {
                $ticket = $info[1]->ticket;
    ...
    public function get_user_info($open_id, $lang = '')
    {
       ...
       // $url="https://api.weixin.qq.com/cgi-bin/user/info?access_token=9_e6Qtl5whNuMEnLq3bMpbokIyxa9f-bX8v9fSa910KGtFUFpRcE23Rw9kdv-8Pwc35EoJHpRaPBdCT5o0MB88KsdjRyYSdpW2QYPXLood2ZNVs1S54_msGkqzK3GFNaZLaJVaZ2YNb7LHOJcqNJGjAFAAWT&openid=oIF791TNJf-TGrK14TPe5jym41pI";
        $res = HttpCurl::get($url, 'json');
        // 异常处理: 获取时网络错误
        if ($res === false) {
            return Error::code('ERR_POST');
     ...
     * Examples:
     * ```
     * $api->batch_get_user_info('ocNtAt_K8nRlAdmNEo_R0WVg_rRw');
     * $api->batch_get_user_info('ocNtAt_K8nRlAdmNEo_R0WVg_rRw', 'zh_TW');
     * ```
     * Result:
     * ```
     * [
     ...
     * - `res`, 调用正常时得到的对象
     */
    public function batch_get_user_info($open_list, $lang = '')
    {
        $user_list = [];
        foreach($open_list as $value){
            $user_list[] = [
                'openid' => $value,
                'lang' => !empty($lang) ? $lang : 'zh_CN', 
            ];
        }
        $xml = sprintf('{"user_list":%s}', json_encode($user_list));

        $url = self::API_DOMAIN . 'cgi-bin/user/info/batchget?access_token=' . $this->get_access_token();
       // $url="https://api.weixin.qq.com/cgi-bin/user/info?access_token=9_e6Qtl5whNuMEnLq3bMpbokIyxa9f-bX8v9fSa910KGtFUFpRcE23Rw9kdv-8Pwc35EoJHpRaPBdCT5o0MB88KsdjRyYSdpW2QYPXLood2ZNVs1S54_msGkqzK3GFNaZLaJVaZ2YNb7LHOJcqNJGjAFAAWT&openid=oIF791TNJf-TGrK14TPe5jym41pI";
        $res = HttpCurl::post($url, $xml, 'json');
        // 异常处理: 获取时网络错误
        if ($res === false) {
        ...
        if (isset($_GET['code']) && !empty($_GET['code'])) {
            ...
            if (isset($res->access_token)) {
                if ($scope == 'snsapi_userinfo') {
                    ...
                    //$url = self::API_DOMAIN . 'sns/userinfo?access_token=' . $this->get_access_token() .'&openid=' . $res->openid . '&lang=' . $lang;
                    $url = self::API_DOMAIN . 'cgi-bin/user/info?access_token=' . $this->get_access_token() .'&openid=' . $res->openid . '&lang=' . $lang;
    ...
    public  function refund($conf = []) {
        ...
        // [必填]商户系统内部的退款单号，同一退款单号多次请求只退一笔。
        //$conf['out_refund_no'] =  date("YmdHis");

        //[必填]商户订单号,pay_sn码 1.1二选一,微信生成的订单号，在支付通知中有返回
        //-out_trade_no
        ...
        /*
        if($this->sslrootcatPath!=false)
        {
            curl_setopt($ch, CURLOPT_SSLCERTTYPE, 'pem');
            curl_setopt($ch, CURLOPT_CAINFO, $this->sslrootcatPath);
        }
        */
     ...
     * @return bool
     */
    function api_add_template($template_id_short){
        if (empty($template_id_short)) {
            return false;
        }
        $json = json_encode(array("template_id_short"=>$template_id_short),JSON_UNESCAPED_UNICODE);
        $url = self::API_DOMAIN . 'cgi-bin/template/api_add_template?access_token=' . $this->get_access_token();
        $res = HttpCurl::post($url, $json, 'json');
        if($res == false){
            return false;
        }
     ...
     * @return mixed
     */
    function get_all_private_template(){
        $url = self::API_DOMAIN . 'cgi-bin/template/get_all_private_template?access_token=' . $this->get_access_token();
        $res = HttpCurl::post($url,"", 'json');
        if($res == false){
            return false;
        }
        return $res;
     ...
     * @return bool
     */
    function del_private_template($template_id)
    {
        if (empty($template_id)) {
            return false;
        }
        $json = json_encode(array("template_id" => $template_id), JSON_UNESCAPED_UNICODE);
        $url = self::API_DOMAIN . 'cgi-bin/template/del_private_template?access_token=' . $this->get_access_token();
        $res = HttpCurl::post($url, $json, 'json');
        if ($res == false) {
            return false;
        }
    ...
    function get_industry(){
        $url = self::API_DOMAIN . 'cgi-bin/template/get_industry?access_token=' . $this->get_access_token();
        Log::info($url);
        $res = HttpCurl::post($url,"", 'json');
        if($res == false){
            return false;
