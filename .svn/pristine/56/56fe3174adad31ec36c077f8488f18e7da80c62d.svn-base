...
class Base extends Controller{
    ...
    function __construct(){
        ...
        if(!empty($request['sitecode'])) {
            ...
            if(empty(session("UserInfo_siteid")))
            {
                $this->ClearSeaaionInfo();
            }
            else if(session("UserInfo_siteid")!=$data['id'])
            {
                $this->ClearSeaaionInfo();
            }
    ...
    public function ClearSeaaionInfo()
    {
        session("UserInfo_nickname",null);
        session("UserInfo_openid",null);
        session("UserInfo_ismanage",null);
        session("UserInfo_userid",null);
        session("UserInfo_siteid",null);
        session("UserInfo_load",null);
    }
