...
//            }
        ...
        $obj = $this->errorCode($obj);
        //记录日志
        \think\Log::info('请求调用人脸的地址：'.$url.'请求的参数:' . json_encode($data,JSON_UNESCAPED_UNICODE) . '---接口数据返回' . json_encode($obj,JSON_UNESCAPED_UNICODE));
        return $obj;
    }
    ...
    private function getAuthFilePath(){
        $path = dirname(__FILE__) . DIRECTORY_SEPARATOR . md5($this->apiKey);
        if(!is_dir($path)) mkdir($path, 0777,true);
//        halt(dirname(__FILE__) . DIRECTORY_SEPARATOR . md5($this->apiKey));
        return $path;
    }
...
//        }
     ...
     * @return mixed
     */
    public function errorCode($obj){
        if($obj){
            $obj['error_chinese_msg'] = array_key_exists($obj['error_code'],config('face_err_code'))?config('face_err_code')[$obj['error_code']]:'找不到错误信息';
        }
        return $obj;
    }
