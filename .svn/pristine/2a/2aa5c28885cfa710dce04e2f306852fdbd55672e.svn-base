...
//        var_dump($result);exit;
    ...
    public function spokesman_activity($request){
        ...
        //查询数据
        $data = db('activity')->where($Search_arr1)->order('chkcontentlev desc,contentlevtime desc,dtpublishtime desc')->limit($page->firstRow.','.$page->pageSize)->select();
        if($data){
            foreach ($data as $key=>$val){
                ...
                $spokesman_activity = db('spokesman_activity')->field('sum(spokesman_pay_num) as spokesman_pay_num')->where(['activity_id'=>$val['idactivity']])->find();
                $data[$key]['spokesman_pay_num'] = $spokesman_activity['spokesman_pay_num'];
            }
        }
//        var_dump($data);exit;
