...
class ReadHtmlCacheBehavior{
    ...
    public function run(&$params){
        // 开启静态缓存
        if($params->isGet() && config('HTML_CACHE_ON'))  {
            $cacheTime = $this->requireHtmlCache($params);
            if( false !== $cacheTime && $this->checkHTMLCache(HTML_FILE_NAME,$cacheTime)) { //静态页面有效
                // 读取静态页面输出
                //echo HTML_FILE_NAME;
                $HTMLCache = cache('HTMLCache_'.HTML_FILE_NAME);
                echo $HTMLCache['cache'];
                exit();
            }else{
                //echo "缓存失效或未生成\n";
            }
    ...
    // 判断是否需要静态缓存
    static private function requireHtmlCache($params) {
        // 分析当前的静态规则
        ...
        if(!empty($htmls)) {
            ...
            if(!empty($html)) {
                ...
                // {ID|FUN} GET变量的简写
                $rule     = preg_replace_callback('/{(\w+)\|(\w+)}/', function($match)use($params){return $match[2]($params->param($match[1]));}, $rule);
                $rule     = preg_replace_callback('/{(\w+)}/', function($match)use($params){return $params->param($match[1]);}, $rule);
                // 特殊系统变量
                $rule   = str_ireplace(
                    array('{:controller}','{:action}','{:module}'),
                    array($params->controller(),$params->action(),$params->module()),
                    $rule);
                // {|FUN} 单独使用函数
                $rule  = preg_replace_callback('/{|(\w+)}/', function($match){return $match[1]();},$rule);
                $cacheTime  =   config('HTML_CACHE_TIME',null,60);
                if(is_array($html)){
                ...
                // 当前缓存文件
                define('HTML_FILE_NAME',$params->module()."/".$rule.config('HTML_FILE_SUFFIX'));
                return $cacheTime;
            }
        }
        // 无需缓存
    ...
    static public function checkHTMLCache($cacheFile='',$cacheTime='') {
        $HTMLCache = cache('HTMLCache_'.$cacheFile);
        if($HTMLCache){
            if($HTMLCache['mtime']+$cacheTime>time()){
                return true;
            }else{
                // 文件是否在有效期
                return false;
