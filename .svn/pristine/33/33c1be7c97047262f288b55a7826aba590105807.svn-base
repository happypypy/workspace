...
class Rolesite extends Model{
    ...
    public function index(){
        ...
        $count = db('role')->where('idsite=:idsite' ,['idsite'=>$this->siteid])->count();
        $page = new Page($count,PAGE_SIZE);
        $role = db('role')->where('idsite=:idsite' ,['idsite'=>$this->siteid])->order('idrole desc')->limit($page->firstRow.','.$page->pageSize)->select();
        $arr = array();
        $arr['pager'] = $page;
        $arr['data'] = $role;
        return $arr;
    ...
    public function role_deal($data){
        ...
        if(array_key_exists('id',$data)){
            $role = db('role')->where('idsite=:idsite and idrole=:idrole',['idsite'=>$this->siteid,'idrole'=>$data['id']])->find();
        }else{
            $role = [
                'rolename'      =>  '',
                'roleremark'    =>  '',
    ...
    public function role_post($data){
        ...
        if($data['action'] == 'add'){
            ...
            $bool = db('role')->where('idsite=:idsite and idrole=:idrole',['idsite'=>$this->siteid,'idrole'=>$data['roleid']])->update($data_arr);
        }
        //修改账户权限状态
        db('account')->where(array('siteid'=>$this->siteid) )->setField('repurview',1);
    ...
    public function role_del($data){
        $bool = db('role')->where('idsite=:idsite and idrole=:idrole',['idsite'=>$this->siteid,'idrole'=>$data['id']])->delete();
        return $bool;
    }
    ...
    public function del_checked($data){
        if(strstr($data['id'],',')){
            ...
            for ($i=0;$i<count($id);$i++){
                $bool = db('role')->where('idsite=:idsite and idrole=:idrole',['idsite'=>$this->siteid,'idrole'=>$id[$i]])->delete();
            }
        }else{
            $bool = db('role')->where('idsite=:idsite and idrole=:idrole',['idsite'=>$this->siteid,'idrole'=>$data['id']])->delete();
        }
    ...
    public function column_info($data){
        ...
        $column_name = db('catalog')->where('intflag=2')->select();
        $module = db('module')->where('intflag=2')->where('idsite','in',"0,$this->siteid") ->select(); //栏目下的所有模块
