...
class Ueditor extends Base
{
    ...
    {       
        ...
        //.jpg;*.jpeg;*.png;*.gif;*.jpeg
        if(true !== $result){            
            $state = "ERROR" . $result;
        }else{
            // 移动到框架应用根目录/public/uploads/ 目录下

            $filepath= $this->savePath.date('Y').'/'.date('m-d').'/';

            if(empty(session('idsite'))==false)
                $filepath= (session('idsite').'/'. $filepath);

            $filepath='public/uploads/'.$filepath;
            $filename = true;
            if($this->savePath=="cart/")
            {
                $filepath="../cart/";
            }
            if($this->savePath=="verify/")
            {
                $filepath="../public/";
                $info = $file->getInfo();
                $filename = $info["name"];
           ...
           //echo print_r($info,true);              
            ...
            if($this->savePath=="cart/")
            {
                $return_data['url'] =$info->getSaveName();
            }elseif($this->savePath=="verify/") {
                $return_data['url'] = $info->getSaveName();
            }
            else
            {
                $return_data['url'] ="/".$filepath.$info->getSaveName();
            }
