...
class activity extends Basesite {
	...
	public function modi() {
        ...
        $this->assign('is_cashed',checkedMarketingPackage(session('idsite'),'cashed'));//是否具有现金券营销包
     ...
     */
    public function reservecheck(){
        $data = Request::instance()->param();

        $obj = new \app\admin\module\activity(session('idsite'));
        $result = $obj->reservecheck($data['id']);
        if($result){
            echo 1;
        }else{
            echo 0;
    ...
    public function setreserve(){
        ...
        $obj = new \app\admin\module\activity(session('idsite'));
        $packagelist = $obj->packagelist($id);
        $cardnamelist = $obj->cardnamelist($id);

        $this->assign('cardnamelist',$cardnamelist);
     ...
     * @return void
     * @author Chenjie
     * @Date 2019-09-06 09:43:31
     */
    public function selectsubproduct(){
        $param = input('param.');

        $obj = new \app\admin\module\activity(session('idsite'));
        $result = $obj->subproductlist($param);
                ...
                if($data)
                {
                    ...
                    foreach ($data as $k=>$vo)
                    {
                        ...
                        else
                        {
                            ...
                            $result = $obj->addOrder($id,$UserName,$payname,$price,$payCount,$txtfield,$txtdata,$source,4,4);
                            ...
                            if($result && $mobile){
                                if($request["action"] == 'import_sms'){
									...
									$activity = db('activity')->where(["idactivity"=>$id])->find();
                                    ...
                                    $msg_content = $trueName.'，您购买的'.$activity['chrtitle'].'兑换码：'.$checkcode.'，请在有效期内完成兑换，兑换操作详情点击：'.$url;
...
//        for($i=0)
		...
		for ($i = 0; $i < $len; $i++) {
			...
			$PHPSheet->setCellValue('B' . ($i + 2), $list[$i]['chrtitle']); //添加产品名称
		...
		//8.设置当前激活的sheet表格名称；
		$objPHPExcel->getActiveSheet()->setTitle("产品报名信息");

		//4.激活当前的sheet表
		$objPHPExcel->setActiveSheetIndex(0);
		//9.设置浏览器窗口下载表格
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		$filename = urlencode("产品报名信息") . date('ymd', time()) . '.xlsx';
...
//                    var_dump($abilitytagsname);exit;
		...
		$this->assign('fiddistrict', $fiddistrict); //产品商圈
		$this->assign('fidarea', $fidarea); //产品区域
