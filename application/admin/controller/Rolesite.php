...
class Rolesite extends Base {
    ...
    public function index(){
        ...
        $obj = new \app\admin\module\Rolesite(session('idsite'));
        $arr = $obj->index();
        $role_list = $arr['data'];
        $page = $arr['pager'];
    ...
    public function roledeal(){
        ...
        $role_obj = new \app\admin\module\Rolesite(session('idsite'));
   ...
   /* //角色权限设置
    ...
    public function purviewpost(){
        ...
        if($bool !== false){
            $this->success('操作成功',url('admin/rolesite/index'));
        }else{
            $this->error('操作失败');
        }
    }
