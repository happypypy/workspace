...
class Base extends Controller{
    ...
    public function _initialize(){
        //empty(session('idsite')) ? $this->idsite = 1 : $this->idsite = session('idsite');
        ...
        if(!in_array(ACTION_NAME,array('login','logout','vertify','forget_pwd','loginsite','tncode','checktn')))
        {
...
//                header("Location:".url("admin/index/loginsite"));
       ...
       /* if(cookie('lang_type')){
        ...
        if(in_array(ACTION_NAME,array('login','logout','vertify','forget_pwd','loginsite','tncode','checktn')) || in_array(CONTROLLER_NAME,array('Ueditor','Uploadify','Table'))){
        	//return;
        }else{
        	if(session('AccountID') > 0 ){
                ...
                echo "<script>if(window.top.document.getElementById('pFrame')){window.top.location.href= '".url('Admin/index/loginsite')."';}else{window.parent.parent.location.href='".url('Admin/index/loginsite')."';}</script>";
                exit;
//        		$this->error('请先登陆',url('Admin/index/loginsite'),1);
        	}
        }
        $this -> operate_log();
    }
    ...
    {
        if(empty(session('idsite')) || session('idsite')<1) {
